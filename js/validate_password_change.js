		...
		//This section checks for empty fields as the form is being filled out 
			...
			$(':input#currentPassword').blur(function(){    // check when cursor moves out of field that it isn't empty
					value = $(this).val();
					if (value == "") {                // if the value is empty
						$(this).addClass('error');    // turns on the error class
						$('#currentError').show();      // shows the error message 
					} else {
						$(this).removeClass('error'); // turns off the error class
						$('#currentError').hide();      // turns off the error message
					} // end if
			});//end each
